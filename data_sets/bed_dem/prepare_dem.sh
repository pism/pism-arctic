...
for grid in 250 500 1000 2000 5000 10000 20000 40000; do
    ...
    gdalwarp $options -dstnodata 1 -cutline ../shape_files/${localdomain}-domain.shp pism_${domain}_v${v}_g${grid}m_thickness.nc  ${localdomain}_v${v}_g${grid}m_mask.nc
    ncatted -a _FillValue,Band1,d,,  ${localdomain}_v${v}_g${grid}m_mask.nc
    ncap2 -4 -L 2 -O -s "ftt_mask=Band1*0; where(Band1==1) ftt_mask=1; thickness=Band1*0;"  ${localdomain}_v${v}_g${grid}m_mask.nc  ${localdomain}_v${v}_g${grid}m_mask.nc
    ncatted  -a units,thickness,o,c,"m" -a standard_name,thickness,o,c,"land_ice_thickness"  ${localdomain}_v${v}_g${grid}m_mask.nc
    ncks -O -v ftt_mask,thickness ${localdomain}_v${v}_g${grid}m_mask.nc ${localdomain}_v${v}_g${grid}m_mask.nc
